...
BREAKING CHANGE: Asset processing hooks in Compilation has been merged into a single Compilation.hooks.processAssets hook.`;
		...
		/** @type {Set<Module>} */
		// TODO@HZL What's is the use of `modules`.
		 ...
		 * Key is module identifier
		 * TODO@HZL What's is the use of `_modules`.
		 */
	...
	_addModule(module, callback) {
		...
		this._modulesCache.get(identifier, null, (err, cacheModule) => {
			...
			// Place store the created module into memory.
	...
	/**
	 * Method recursively build the dependencies of the given module into module.
	 * @param {Module} module to be processed for deps
	 * @param {ModuleCallback} callback callback to be triggered
	 * @returns {void}
	 */
	_processModuleDependencies(module, callback) {
		/**
		 * The dependencies to be convert into module.
		...
		const processDependencyForResolving = dep => {
			...
			if (resourceIdent !== undefined && resourceIdent !== null) {
				...
				if (list === undefined) {
					...
					// Put dependencies to be processed into this array.
	 ...
	 * @property {Dependency[]} dependencies The dependency to create from
	...
	handleModuleCreation(
		...
		// NOTE@HZL Create module from `dependencies`.
		this.factorizeModule(
			...
			(err, factoryResult) => {
				...
				// This is not addEntry.
				// NOTE@HZL Module is created, but haven't build (read file content, parse, etc.),
				// then we need to store it into `moduleGraph` and some other places.
	 ...
	 * @param {ModuleOrFactoryResultCallback} callback The callback defined at @see {Compilation.handleModuleCreation} https://github.com/BilderLoong/webpack/blob/97002a159224950434ad82c5f6071a7bbd0e02d1/lib/Compilation.js#L1775
	 * @returns {void}
	 */
	_factorizeModule(
		{
		...
		// NOTE@HZL place use `NormalModuleFactory` to create `Module`.
		factory.create(
			{
				contextInfo: {
	...
	/**
	 * [这个函数的主要作用就是通过 _addEntryItem 添加入口，因为编译需要从入口开始](https://github.com/gweid/webpack-source-code#3compilation-%E5%AF%B9%E6%A8%A1%E5%9D%97%E7%9A%84%E5%A4%84%E7%90%86)
	...
	_addEntryItem(context, entry, target, options, callback) {
		...
		// Add current module into module tree.
		this.addModuleTree(
			{
				context,
				dependency: entry, // The current module to add.
