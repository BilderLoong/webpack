...
class NormalModuleFactory extends ModuleFactory {
	...
	constructor({
		...
		this.hooks = Object.freeze({
			...
			/**
			 * This hook will fired when create new module.
		...
		this.hooks.factorize.tapAsync(
			...
			(resolveData, callback) => {
				// NOTE@HZL `resolveData` is modified implicitly when running this hook.
			...
			(data, callback) => {
				...
				const continueCallback = needCalls(2, err => {
					...
					const continueCallback = needCalls(3, err => {
						...
						try {
							// NOTE@HZL Place set the `NormalModule`'s constructor parameter.
	...
	/**
	 * @param {*} unresolvedResource The resource to resolve.
	 */
	resolveResource(
		contextInfo,
		context,
		...
		// NOTE@HZL Place use enhance-resolver to resolve the request resource.
		resolver.resolve(
			contextInfo,
			context,
			unresolvedResource,
